...
net_packet_t *NET_NewPacket(int initial_size)
{
    ...
    packet = Z_Malloc<decltype(packet)>(sizeof(net_packet_t), PU_STATIC, 0);
    
    if (initial_size == 0)
        initial_size = 256;

    packet->alloced = initial_size;
    packet->data = Z_Malloc<decltype(packet->data)>(initial_size, PU_STATIC, 0);
    packet->len = 0;
    packet->pos = 0;
...
static void NET_IncreasePacket(net_packet_t *packet)
{
    ...
    newdata = Z_Malloc<byte *>(packet->alloced, PU_STATIC, 0);
