...
MEMFILE *mem_fopen_read(void *buf, size_t buflen)
{
	...
	file = Z_Malloc<MEMFILE *>(sizeof(MEMFILE), PU_STATIC, 0);
...
MEMFILE *mem_fopen_write(void)
{
	...
	file->alloced = 1024;
	file->buf = Z_Malloc<decltype(file->buf)>(file->alloced, PU_STATIC, 0);
	file->buflen = 0;
	file->position = 0;
...
size_t mem_fwrite(const void *ptr, size_t size, size_t nmemb, MEMFILE *stream)
{
	...
	while (bytes > stream->alloced - stream->position)
	{
		...
		newbuf = Z_Malloc<decltype(newbuf)>(stream->alloced * 2, PU_STATIC, 0);
